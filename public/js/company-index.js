...
$(document).ready(function () {
    ...
    // Modals
    const modalShow = new bootstrap.Modal(document.getElementById('modalShowCompany'));
    const modalEdit = new bootstrap.Modal(document.getElementById('modalEditCompany'));

    loadDataTables().then(function () {
        console.log('✅ DataTables loaded successfully');

        // Initialize DataTables
        const table = $('#company-datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                    ...
                    render: function (data, type, row) {
                        ...
                        <div class="action-icons">
                            ...
                            <button
                                ...
                                data-item-id="${row.id}"
                                data-item-name="${row.name || 'esta empresa'}"
                                title="Eliminar"
                            >
        ...
        // View Button Handler
        $('#company-datatable').on('click', '.btn-view', function () {
            ...
            fetch(`/${config.dominio}/company/${itemId}/details`)
                ...
                .then(data => {
                    ...
                    // Populate Modal
                    document.getElementById('showCompanyName').value = data.name;
                    document.getElementById('showCompanyRegion').value = data.regionName || 'Sin región';
                    document.getElementById('showCompanyStatus').value = data.status;
        ...
        // Edit Button Handler
        $('#company-datatable').on('click', '.btn-edit', function () {
            const itemId = $(this).data('item-id');
            ...
            fetch(`/${config.dominio}/company/${itemId}/details`)
                ...
                .then(data => {
                    ...
                    form.action = `/${config.dominio}/company/${itemId}/edit`;

                    document.getElementById('editCompanyName').value = data.name;

                    // Set Region Select
                    const regionSelect = document.getElementById('editCompanyRegion');
                    if (data.regionId) {
                        regionSelect.value = data.regionId;
                    } else {
                        regionSelect.value = "";
                    }
        ...
        $('#company-datatable').on('click', '.action-icon-delete', function () {
            const itemId = $(this).data('item-id');
            ...
            Swal.fire({
                ...
                fetch(`/${config.dominio}/company/${itemId}/delete`, {
                    method: 'POST',
                    headers: {
